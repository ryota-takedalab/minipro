...
def feature_extraction(path_list, event_list):
    ...
    features_array = np.zeros((n_dataset, 2 * N_FRAME, 62))
    
    for path_pos, path_rot, event, i in zip(path_list_pos, path_list_rot, event_list, range(n_dataset)):
        pos = pd.read_csv(path_pos, usecols=["time","Hips.x","Hips.y","Hips.z","RightUpLeg.x","RightUpLeg.y","RightUpLeg.z","RightLeg.x","RightLeg.y","RightLeg.z","RightFoot.x","RightFoot.y","RightFoot.z","LeftUpLeg.x","LeftUpLeg.y","LeftUpLeg.z","LeftLeg.x","LeftLeg.y","LeftLeg.z","LeftFoot.x","LeftFoot.y","LeftFoot.z","Spine.x","Spine.y","Spine.z","Spine1.x","Spine1.y","Spine1.z","Spine2.x","Spine2.y","Spine2.z"])
        rot = pd.read_csv(path_rot, usecols=["time","Hips.x","Hips.y","Hips.z","RightUpLeg.x","RightUpLeg.y","RightUpLeg.z","RightLeg.x","RightLeg.y","RightLeg.z","RightFoot.x","RightFoot.y","RightFoot.z","LeftUpLeg.x","LeftUpLeg.y","LeftUpLeg.z","LeftLeg.x","LeftLeg.y","LeftLeg.z","LeftFoot.x","LeftFoot.y","LeftFoot.z","Spine.x","Spine.y","Spine.z","Spine1.x","Spine1.y","Spine1.z","Spine2.x","Spine2.y","Spine2.z"])
        pos = pos.iloc[event - N_FRAME: event + N_FRAME, ]
        rot = rot.iloc[event - N_FRAME: event + N_FRAME, ]
        pos = pos.rename(columns=lambda s: s + '_pos')
        rot = rot.rename(columns=lambda s: s + '_rot')
        feature = pd.merge(pos, rot, left_on='time_pos', right_on='time_rot')
        feature_array = feature.to_numpy()
        features_array[i] = feature_array
